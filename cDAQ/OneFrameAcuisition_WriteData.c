...
int main(void)
{
	...
	DAQmxErrChk (DAQmxCfgSampClkTiming(taskHandle,"",Freq,DAQmx_Val_Rising,DAQmx_Val_FiniteSamps,512));
	DAQmxErrChk (DAQmxRegisterEveryNSamplesEvent(taskHandle,DAQmx_Val_Acquired_Into_Buffer,512,0,EveryNSamplesCallback,NULL));
...
int32 CVICALLBACK EveryNSamplesCallback(TaskHandle taskHandle, int32 everyNsamplesEventType, uInt32 nSamples, void *callbackData)
{
	...
	// DAQmx Read Code
	/*********************************************/
	DAQmxErrChk(DAQmxReadAnalogF64(taskHandle,-1,10.0,DAQmx_Val_GroupByChannel,data,512,&read,NULL));
	...
	{   
		...
		{
			lines=totalRead/512;
			Imagedata[i+totalRead]=(int16)(data[i]*32767)+32768;
