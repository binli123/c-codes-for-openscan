...
static float64		data[512*Binfactor];
static int16		Image[512];
static int16		*Imagedata=NULL;
...
int main(void)
{
	...
	int32		triggersize=datasize-1000;
	float64		Acq_freq=Freq/1000;
	char    trigName[256];
	uInt8		 *Acq_data=NULL;

	Imagedata=(int16*)malloc(sizeof (int16) *512*512);
	...
	fileID = fopen("Acquisition.txt","w");
	...
	//Configure acquisition trigger info 
	DAQmxErrChk (DAQmxCreateTask("",&Acq_taskHandle));
	DAQmxErrChk (DAQmxCreateDOChan(Acq_taskHandle,"PXI1Slot2/port0/line5","",DAQmx_Val_ChanPerLine));
	DAQmxErrChk (DAQmxCfgSampClkTiming(Acq_taskHandle,"",Freq,DAQmx_Val_Rising,DAQmx_Val_FiniteSamps,triggersize));
	...
	DAQmxErrChk (DAQmxCreateAIVoltageChan(taskHandle,"PXI1Slot2/ai0","",DAQmx_Val_Cfg_Default,-10.0,10.0,DAQmx_Val_Volts,NULL));
	DAQmxErrChk (DAQmxCfgSampClkTiming(taskHandle,"",Freq*Binfactor,DAQmx_Val_Rising,DAQmx_Val_FiniteSamps,512*Binfactor));
	DAQmxErrChk (DAQmxRegisterEveryNSamplesEvent(taskHandle,DAQmx_Val_Acquired_Into_Buffer,512*Binfactor,0,EveryNSamplesCallback,NULL));
	DAQmxErrChk (DAQmxCfgDigEdgeStartTrig(taskHandle,"/PXI1Slot2/PFI7",DAQmx_Val_Rising));
	...
	//fwrite(Imagedata,sizeof(int16),262144,fileID);
...
int32 CVICALLBACK EveryNSamplesCallback(TaskHandle taskHandle, int32 everyNsamplesEventType, uInt32 nSamples, void *callbackData)
{
	...
	int32		lines=0;
	int32		bin=0;
	float64		Averagedata[resolution]={0};
	...
	DAQmxErrChk(DAQmxReadAnalogF64(taskHandle,-1,10.0,DAQmx_Val_GroupByChannel,data,512*Binfactor,&read,NULL));

	
		fprintf(fileID,"\n");
		/*for(i=0;i<(512*Binfactor);i++)
		{
			if(data[i]<1e-8)
				data[i]=-data[i];
		}*/
		for(i=0;i<512*Binfactor;i=i+Binfactor)
		{
			//lines=totalRead/512;
			//printf("data[%d]=%d\n",i,Imagedata[i+totalRead]);
			for(bin=0;bin<Binfactor;bin++)
				Averagedata[i/Binfactor]+=data[i+bin];
			Averagedata[i/Binfactor]=Averagedata[i/Binfactor]/Binfactor;
			Image[i/Binfactor]=(int16)((Averagedata[i/Binfactor])/2*32767);
			
			if(Image[i/Binfactor]<0)
				Image[i/Binfactor]=-1*Image[i/Binfactor];
			fprintf(fileID,"%d\t",Image[i/Binfactor]);
		}
		
		//fprintf(fileID,"%d\t",Image[i]);
		//fwrite(Imagedata,sizeof(int16),512,fileID);
		if( read>0 )   
			printf("Acquired %d samples. Line:%d,Total %d\n\r",(int)read,(int)(totalRead/(512*Binfactor)),(int)(totalRead+=read));
		if(totalRead==512*512*Binfactor)
			{
				printf("End of imaging");
			}
		fflush(stdout);
